<template>
	<div>
		<div v-if="raffles.length !== 0" class="raffleall_mobile">
			<div class="site_count">응모 사이트 {{ raffles.length }}개</div>
			<div class="title">
				{{ detailraffle.product_name }}
				<br />응모 사이트
			</div>
			<div class="sorted">
				<select name="methods" class="m-2 method2">
					<option value="">응모방법</option>
					<option value="">준비중</option>
				</select>

				<select name="methods2" class="method">
					<option value="">배송국가</option>
					<option v-for="country in raffle_country" :key="country.id">{{
						country
					}}</option>
				</select>
			</div>
			<b-container>
				<template v-if="cutRaffle.length">
					<b-row
						v-for="(list, i) in cutRaffle"
						:key="list.id"
						class="raffle_item"
					>
						<div
       ...
       index:999"
			...
			<div v-if="!viewflag" class="view_more" @click="addCount()">
				더보기
			</div>
			<div style="clear:both"></div>
		</div>

		<div v-else style="padding-top:15.8%">
			<client-only>
				데이터가 존재하지 않습니다.
			</client-only>
		</div>
...
export default {
	...
	props: { detailraffle: { type: Object, required: true } },
	data() {
		return {
			raffles: '',
			raffle_country: [],
	...
	created() {
		if (this.detailraffle.raffles.length > 0) {
			for (let i = 0; i < this.detailraffle.raffles.length; i++) {
				this.detailraffle.raffles[i].props = this.getRaffleStatus(i);
			}
		}
	},
	mounted() {
		this.raffles = this.detailraffle.raffles;
		this.addCount();
		this.addCountry();
		this.sortByOnRaffle();
	},
	methods: {
		getRaffleStatus(index) {
			const raffle = this.detailraffle.raffles[index];
			const startDate = raffle.start_date;
		...
		addCount() {
			const numCountTmp = this.numCount;

			if (this.numCount + 10 > this.raffles.length) {
				this.numCount = this.raffles.length;
				this.viewflag = true;
			} else {
				this.numCount += 10;
			}
			for (let i = numCountTmp; i < this.numCount; i++) {
				this.cutRaffle.push(this.raffles[i]);
			}
		},
		sortByOnRaffle() {
		...
		addCountry() {
			this.raffle_country = [];
			for (let i = 0; i < this.numCount; i++) {
				const country = this.raffles[i].shop.country;
				if (country !== undefined && country.length > 0) {
					if (!this.raffle_country.includes(country)) {
						this.raffle_country.push(this.raffles[i].shop.country);
					}
				}
			}
		},
