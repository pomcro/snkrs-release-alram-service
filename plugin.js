...
function getSpritePath(sprite) {
	const module = require('<%= relativeToBuild(options._output) %>/' +
		sprite +
		'.svg');
...
function getIcon(info) {
	...
	return '/assets/svg/' + sprite + '.svg' + `#i-${generateName(icon)}`;
}
...
const SvgIcon = {
	...
	render(h, { props, data }) {
		...
		const componentData = {
			class:
				'<%= options.elementClass %> <%= options.spriteClassPrefix %>' + sprite,
			attrs: {
